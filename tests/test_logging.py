...
def test_logging_log_levels_trace(pytester: pytest.Pytester, page: Page) -> None:
    ...
    expect(log_lines.locator("td.msg-cell")).to_have_text(
...
def test_logging_span_simple(pytester: pytest.Pytester, page: Page) -> None:
    ...
    span_content = page.get_by_role(
        "cell", name="This is an INFO log message inside a span."
    )
...
def test_logging_span_critical_propagates(
...
def test_logging_log_fixtures_teardown(pytester: pytest.Pytester, page: Page) -> None:
    ...
    expect(
        test_teardown.locator("td.msg-cell").filter(
            has_text="Tore down fixture sandwich()"
        )
    ).to_have_count(1)
    expect(
        test_teardown.locator("td.msg-cell").filter(
            has_text="Tore down fixture foobulator()"
        )
    ).to_have_count(1)
